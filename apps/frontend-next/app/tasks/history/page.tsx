  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-200 pb-20 md:pb-8">
      <div className="max-w-screen-md mx-auto px-4 md:px-6 py-4 md:py-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-50 mb-6 md:mb-8">Мои задачи</h1>
        ...
        <div className="mb-6 border-b border-gray-200 dark:border-slate-700">
          <nav className="flex">
            <button
              ...
              className={`flex-1 md:flex-none py-3 md:py-4 px-4 md:px-1 md:mr-8 border-b-2 font-medium text-sm md:text-base transition-colors min-h-[44px] ${
                activeTab === 'created'
            ...
            <button
              ...
              className={`flex-1 md:flex-none py-3 md:py-4 px-4 md:px-1 border-b-2 font-medium text-sm md:text-base transition-colors min-h-[44px] ${
        ...
        {activeTab === 'created' && (
          <section>
            {history?.created.length === 0 ? (
              <div className="card text-center py-12 p-4 md:p-6">
                <div className="w-16 h-16 md:w-20 md:h-20 mx-auto mb-4 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center">
                  <svg className="w-8 h-8 md:w-10 md:h-10 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  >
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <span className={`px-2.5 md:px-3 py-1 rounded-full text-xs md:text-sm font-medium ${urgencyColors[task.urgency]}`}>
                          {urgencyLabels[task.urgency]}
                        </span>
                        <span className="px-2.5 md:px-3 py-1 rounded-full text-xs md:text-sm font-medium bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300">
                          {statusLabels[task.status]}
                        </span>
                      </div>
                    </div>
                    <p className="text-sm md:text-base text-gray-600 dark:text-gray-300 mb-2">{task.fullDescription.substring(0, 100)}...</p>
          ...
          <section>
            {history?.claimed.length === 0 ? (
              <div className="card text-center py-12 p-4 md:p-6">
                ...
                <Link href="/feed" className="btn-primary inline-block w-full md:w-auto">
