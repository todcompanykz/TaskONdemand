...
export class FCMService {
  ...
  private initializeFirebase() {
    try {
      ...
      if (serviceAccountPath) {
        ...
        this.logger.warn(
          'Set FIREBASE_SERVICE_ACCOUNT_PATH or FIREBASE_SERVICE_ACCOUNT_JSON environment variable',
        );
      }
    } catch (error) {
  ...
  async sendNotification(
    ...
    try {
      fetch(
        'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
        {
          ...
          body: JSON.stringify({
            location: 'fcm.service.ts:sendNotification:entry',
            message: 'sendNotification called',
            data: {
              userId,
              title: payload.title,
    ...
    this.logger.log(
      `[DEBUG] sendNotification called: userId=${userId}, hasFirebaseApp=${!!this.firebaseApp}`,
    );
    // #endregion
      ...
      try {
        fetch(
          'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
          {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              location: 'fcm.service.ts:sendNotification:noFirebase',
    ...
    try {
      ...
      try {
        fetch(
          'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
          {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              location: 'fcm.service.ts:sendNotification:userFound',
      ...
      this.logger.log(
        `[DEBUG] User lookup: found=${!!user}, hasFcmToken=${!!user?.fcmToken}`,
      );
      // #endregion
        ...
        try {
          fetch(
            'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
            {
              ...
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                location: 'fcm.service.ts:sendNotification:noToken',
                message: 'user has no FCM token',
                data: { userId, userFound: !!user },
                timestamp: Date.now(),
                sessionId: 'debug-session',
                runId: 'run1',
                hypothesisId: 'I',
      ...
      try {
        fetch(
          'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
          {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              location: 'fcm.service.ts:sendNotification:success',
      ...
      this.logger.log(
        `Successfully sent FCM notification to user ${userId}: ${response}`,
      );
      return true;
    } catch (error) {
      // #region agent log
      try {
        fetch(
          'http://127.0.0.1:7242/ingest/8c69d9e6-e12c-4335-8ddd-7b9bc9b0fafd',
          {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              location: 'fcm.service.ts:sendNotification:error',
